...
// File: contracts/v1-mining/interface/IGameFiGminerV1.sol


pragma solidity 0.7.4;


interface IGameFiGminerV1 {
    ...
    event UpdatePool(bool action, uint256 pool, uint256 startBlock, uint256 rewardTotal, uint256 rewardPerBlock);

    event EndPool(uint256 pool);

    event Stake(uint256 pool, address indexed from, uint256 amount);

    event UpdatePower(uint256 pool, uint256 totalPower, address indexed owner, uint256 ownerPledgePower);

    event UnStake(uint256 pool, address indexed to, uint256 amount);

    event WithdrawReward(uint256 pool, address indexed to, uint256 amount);

    event Mint(uint256 pool, uint256 amount);
    ...
    function updatePledgeUser(uint256, address, uint256) external;
    ...
    function addPool(uint256, uint256, uint256) external returns (bool);
...
// File: contracts/v1-mining/implement/GameFiGminerV1.sol
...
contract GameFiGminerV1 is IGameFiGminerV1 {
    ...
    constructor (ITokenGameFi _gameFi) {
    ...
    function deposit(uint256 _pool, address _user, uint256 _amount) override external {
        require(operateOwner[msg.sender], ErrorCode.FORBIDDEN);//only operateOwner
        require(0 < _amount, ErrorCode.FORBIDDEN);
        PoolInfo storage poolInfo = poolInfos[_pool];
        require(poolInfo.startBlock <= block.number, ErrorCode.MINING_NOT_STARTED);
        //require(0 == poolInfo.endBlock, ErrorCode.END_OF_MINING);

        computeReward(_pool);

        provideReward(_pool, poolInfo.rewardPerShare, _user);

        addPower(_pool, _user, _amount);

        setRewardDebt(_pool, poolInfo.rewardPerShare, _user);

        emit Stake(_pool, _user, _amount);
    }

    function harvest(uint256 _pool) override external {
        ...
        provideReward(_pool, poolInfo.rewardPerShare, msg.sender);
    ...
    function withdraw(uint256 _pool, uint256 _amount) override external {
        ...
        if (0 < _amount) {
            emit UnStake(_pool, msg.sender, _amount);
        }
    }

    function updatePledgeUser(uint256 _pool, address _user, uint256 _amount) override external {
        require(operateOwner[msg.sender], ErrorCode.FORBIDDEN);//only operateOwner
        PoolInfo storage poolInfo = poolInfos[_pool];
        require(poolInfo.startBlock <= block.number, ErrorCode.MINING_NOT_STARTED);
        ...
        updatePower(_pool, _user, _amount);
    ...
    function addPower(uint256 _pool, address _user, uint256 _amount) internal {
        ...
        emit UpdatePower(_pool, poolInfo.totalPower, _user, userInfo.pledgePower);
    }
    ...
    function updatePower(uint256 _pool, address _user, uint256 _amount) internal {
        UserInfo storage userInfo = pledgeUserInfo[_pool][_user];
        if(_amount > userInfo.amount){
            addPower( _pool,  _user,  _amount.sub(userInfo.amount));
        }else if(_amount < userInfo.amount){
            subPower( _pool,  _user,  userInfo.amount.sub(_amount));
        }
    }

    function provideReward(uint256 _pool, uint256 _rewardPerShare, address _user) internal {
        ...
        if (0 < userInfo.pendingReward) {
            ...
            emit WithdrawReward(_pool, _user, userInfo.pendingReward);
    ...
    function pendingReward(uint256 _pool, address _user) override external view returns (uint256) {
        ...
        PoolInfo memory poolInfo = poolInfos[_pool];
        if (poolInfo.startBlock <= block.number) {
    ...
    function addPool(uint256 _startBlock, uint256 _rewardTotal, uint256 _rewardPerBlock) override external returns (bool) {
        ...
        emit UpdatePool(true, _pool, poolInfo.startBlock, poolInfo.rewardTotal, poolInfo.rewardPerBlock);
    ...
    function setRewardPerBlock(uint256 _pool, uint256 _rewardPerBlock) override external {
        require(operateOwner[msg.sender], ErrorCode.FORBIDDEN);
        PoolInfo storage poolInfo = poolInfos[_pool];
        require(0 == poolInfo.endBlock, ErrorCode.POOL_NOT_EXIST_OR_END_OF_MINING);
        
        computeReward(_pool);
        
        poolInfo.rewardPerBlock = _rewardPerBlock;

        emit UpdatePool(false, _pool, poolInfo.startBlock, poolInfo.rewardTotal, poolInfo.rewardPerBlock);
    }
